...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s stohn-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "stohn-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n" +
                                      "By default stohn-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n" +
